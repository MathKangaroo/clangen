...
class Events:
    ...
    def get_moon_freshkill(self):
        ...
        healthy_hunter = list(
            filter(
                lambda c: c.status in ["warrior", "apprentice", "leader", "deputy", "denkeeper", "denkeeper apprentice", "caretaker apprentice", "messenger apprentice"]
                and not c.dead
                and not c.outside
                and not c.exiled
    ...
    def handle_focus(self):
        ...
        if game.clan.clan_settings.get(
            ...
            healthy_apprentices = list(
                filter(
                    lambda c: c.status in ["apprentice", "denkeeper apprentice", "caretaker apprentice", "messenger apprentice"]
                    and not c.dead
                    and not c.outside
                    and not c.exiled
                    and not c.not_working(),
        ...
        for cat_ID in cat_IDs:
            ...
            if x.status in [
                ...
                if x.moons >= 15:
                    ...
                    elif x.status == "denkeeper apprentice":
                        self.ceremony(x, "denkeeper")
                ...
                if cat.status == "kitten":
                    ...
                    if not has_med_app and not int(random.random() * chance):
                        ...
                        if cat.is_disabled() and game.clan.clan_settings["higher_disabled_med_rates"]:
                            ...
                            gardener_chance = int(gardener_chance / game.config["roles"]["disabled_cat_med_chance_increase"])
                            storyteller_chance = int(storyteller_chance / game.config["roles"]["disabled_cat_med_chance_increase"])
                        ...
                        if (
                            ...
                            self.ceremony(cat, "denkeeper apprentice")
                            self.ceremony_accessory = True
                            self.gain_accessories(cat)
                        elif (
                            messenger_list
            ...
            if cat.status in [
                ...
                if _ready:
                    ...
                    # promote to med cat
                        ...
                        self.ceremony(cat, "denkeeper", preparedness)
                        self.ceremony_accessory = True
                        self.gain_accessories(cat)
    ...
    def ceremony(self, cat, promoted_to, preparedness="prepared"):
        ...
        random_honor = None
        if promoted_to in ["warrior", "mediator", "medicine cat", "messenger", "caretaker", "denkeeper", "gardener", "storyteller"]:
        ...
        if cat.status in ["warrior", "medicine cat", "mediator", "messenger", "caretaker", "denkeeper", "gardener", "storyteller"]:
    ...
    def handle_injuries_or_general_death(self, cat):
        ...
        if game.clan.clan_settings.get("disasters"):
            #if clan has a messenger, decreases chance
            disaster_risk = 10 # 1/1010
            for cat in Cat.all_cats_list:
                if cat.status in ["messenger", "messenger apprentice"]:
                    disaster_risk += 1
                    break
            if not random.getrandbits(disaster_risk):  # 1/1010
                handle_short_events.handle_event(
                    event_type="birth_death",
                    main_cat=cat,
                    random_cat=random_cat,
