...
class ProfileScreen(Screens):
    ...
    def build_profile(self):
        ...
        elif self.the_cat.status.rank in [CatRank.CARETAKER, CatRank.CARETAKER_APPRENTICE]:
    ...
    def generate_column1(self, the_cat):
        ...
        if the_cat.dead:
            years2 = round((the_cat.dead_for / 12.0), 1)
            ...
            output += i18n.t("general.years_age", count=years2)
        else:
        ...
        if len(the_cat.enemy) > 0:
            ...
            for _b in the_cat.enemy[:2]:
                ...
                if not isinstance(enemy_ob, Cat):
                    continue
                if enemy_ob.dead != self.the_cat.dead:
                    if the_cat.dead:
            ...
            if len(the_cat.enemy) > 2:
                enemy_block = i18n.t(
                    "utility.items",
                    count=2,
                    item1=enemy_block,
                    item2=i18n.t("general.enemy_extra", count=len(the_cat.enemy) - 2),
                )
