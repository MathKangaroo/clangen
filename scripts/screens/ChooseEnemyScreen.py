...
class ChooseEnemyScreen(Screens):
    def __init__(self, name=None):
        ...
        self.toggle_enemy = None
        self.page_number = None

        self.enemy_frame = None
        ...
        self.enemies_tab_button = None
        self.mates_tab_button = None
        self.besties_tab_button = None
        self.potential_enemies_button = None
        ...
        self.mates_page = 0
        self.besties_page = 0
        self.potential_enemies_page = 0

        self.enemies_cat_buttons = {}
        self.mates_cat_buttons = {}
        self.besties_cat_buttons = {}
        self.potential_enemies_buttons = {}

        # Tab containers.
        self.enemies_container = None
        ...
        self.enemy_page_display = None

        # Keep track of the open tab
        # Can be "potential" for the potential enemies tab, "offspring"
        # for the offspring tab, and "enemies" for the enemy tab.
        ...
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            ...
            if event.ui_element == self.back_button:
                ...
                if self.work_thread is not None and self.work_thread.is_alive():
                    return
                self.work_thread = self.loading_screen_start_work(self.change_enemy)
            ...
            # Next and last page buttons
                ...
                self.potential_enemies_page += 1
                self.update_potential_enemies_container_page()
            elif event.ui_element == self.potential_last_page:
                self.potential_enemies_page -= 1
                self.update_potential_enemies_container_page()
            elif event.ui_element == self.enemies_next_page:
                self.enemies_page += 1
                self.update_enemies_container_page()
            elif event.ui_element == self.enemies_last_page:
                self.enemies_page -= 1
                self.update_enemies_container_page()
            ...
            elif event.ui_element == self.tab_buttons.get("enemies"):
                self.open_tab = "enemies"
                self.switch_tab()
                ...
                event.ui_element in self.enemies_cat_buttons.values()
                or event.ui_element in self.potential_enemies_buttons.values()
            ):
    ...
    def screen_switches(self):
        ...
        self.enemy_frame = pygame_gui.elements.UIImage(
        ...
        self.enemies_container = pygame_gui.core.UIContainer(contain_rect, MANAGER)

        # All the perm elements the exist inside self.enemies_container
        self.enemies_next_page = UISurfaceImageButton(
            ui_scale(pygame.Rect((366, 179), (34, 34))),
            Icon.ARROW_RIGHT,
            get_button_dict(ButtonStyles.ICON, (34, 34)),
            object_id="@buttonstyles_icon",
            container=self.enemies_container,
        )
        self.enemies_last_page = UISurfaceImageButton(
        ...
        self.toggle_enemy = UIImageButton(
    ...
    def change_enemy(self):
        ...
        if self.selected_cat.ID not in self.the_cat.enemy:
            self.the_cat.set_enemy(self.selected_cat)

        else:
            self.the_cat.unset_enemy(self.selected_cat, breakup=True)
    ...
    def update_enemies_container(self):
        """Updates everything in the enemies container, including the list of current enemies,
        and the page"""

        self.all_enemies = self.chunks([Cat.fetch_cat(i) for i in self.the_cat.enemy], 30)
        self.update_enemies_container_page()

    def update_enemies_container_page(self):
        """Updates just the current page for the enemies container, does
        not refresh the list. It will also update the disable status of the
        next and last page buttons"""
        for ele in self.enemies_cat_buttons:
            self.enemies_cat_buttons[ele].kill()
        self.enemies_cat_buttons = {}

        # Different layout for a single enemy - they are just big in the center
        if len(self.all_enemies) == 1 and len(self.all_enemies[0]) == 1:
            # TODO disable both next and previous page buttons
            self.enemies_page = 0
            self.enemies_last_page.disable()
            self.enemies_next_page.disable()
            _enemy = self.all_enemies[0][0]
            self.enemies_cat_buttons["cat"] = UISpriteButton(
                ui_scale(pygame.Rect((240, 13), (150, 150))),
                pygame.transform.scale(_enemy.sprite, ui_scale_dimensions((150, 150))),
                cat_object=_enemy,
                manager=MANAGER,
                container=self.enemies_container,
            )
        ...
        total_pages = len(self.all_enemies)
        if max(1, total_pages) - 1 < self.enemies_page:
            self.enemies_page = total_pages - 1
        elif self.enemies_page < 0:
            self.enemies_page = 0

        if total_pages <= 1:
            self.enemies_last_page.disable()
            self.enemies_next_page.disable()
        elif self.enemies_page >= total_pages - 1:
            self.enemies_last_page.enable()
            self.enemies_next_page.disable()
        elif self.enemies_page <= 0:
            self.enemies_last_page.disable()
            self.enemies_next_page.enable()
        else:
            self.enemies_last_page.enable()
            self.enemies_next_page.enable()

        text = f"{self.enemies_page + 1} / {max(1, total_pages)}"
        if not self.enemy_page_display:
            self.enemy_page_display = pygame_gui.elements.UILabel(
            ...
            self.enemy_page_display.set_text(text)

        if self.all_enemies:
            display_cats = self.all_enemies[self.enemies_page]
        else:
            display_cats = []
        ...
        pos_y = 0
        i = 0
        for _enemy in display_cats:
            self.enemies_cat_buttons["cat" + str(i)] = UISpriteButton(
    ...
    def update_potential_enemies_container(self):
        """Updates everything in the potential enemies container, including the list of current enemies, checkboxes
        and the page"""

        self.all_potential_enemies = self.chunks(self.get_valid_enemies(), 30)
    ...
    def update_potential_enemies_container_page(self):
        """Updates just the current page for the enemies container, does
        not refresh the list. It will also update the disable status of the
        next and last page buttons"""

        for ele in self.potential_enemies_buttons:
            self.potential_enemies_buttons[ele].kill()
        self.potential_enemies_buttons = {}

        total_pages = len(self.all_potential_enemies)
        if max(1, total_pages) - 1 < self.potential_enemies_page:
            self.potential_enemies_page = total_pages - 1
        elif self.potential_enemies_page < 0:
            self.potential_enemies_page = 0

        if total_pages <= 1:
            self.potential_last_page.disable()
            self.potential_next_page.disable()
        elif self.potential_enemies_page >= total_pages - 1:
        ...
        text = f"{self.potential_enemies_page + 1} / {max(1, total_pages)}"
        ...
        if self.all_potential_enemies:
            display_cats = self.all_potential_enemies[self.potential_enemies_page]
        else:
            display_cats = []
        ...
        for _off in display_cats:
            self.potential_enemies_buttons["cat" + str(i)] = UISpriteButton(
                ui_scale(pygame.Rect((pos_x, pos_y), (50, 50))),
                _off.sprite,
                cat_object=_off,
                container=self.potential_container,
    ...
    def exit_screen(self):
        ...
        self.enemies_last_page = None
        self.enemies_next_page = None
        self.potential_page_display = None
        self.enemy_page_display = None
        self.mate_page_display = None
        self.bestie_page_display = None
    ...
    def update_current_cat_info(self, reset_selected_cat=True):
        ...
        if self.the_cat.enemy:
            info += f"\n{len(self.the_cat.enemy)} " + i18n.t(
                "general.enemy", count=len(self.the_cat.enemy)
            )
        ...
        if reset_selected_cat:
            self.selected_cat = None
            if self.the_cat.enemy:
                self.selected_cat = Cat.fetch_cat(self.the_cat.enemy[0])
            self.update_selected_cat()

        self.draw_tab_button()
        self.update_enemies_container()
        self.update_mates_container()
        self.update_besties_container()
        self.update_potential_enemies_container()
    ...
    def draw_tab_button(self):
        ...
        self.tab_buttons["potential"] = UISurfaceImageButton(
            button_rect,
            "screens.choose_enemy.potential",
        ...
        enemies_tab_shown = False
        button_rect.bottomleft = ui_scale_offset((7, 8))
        if self.the_cat.enemy:
            self.tab_buttons["enemies"] = UISurfaceImageButton(
                button_rect,
                "screens.choose_enemy.current",
                get_button_dict(ButtonStyles.HORIZONTAL_TAB, (153, 39)),
                object_id="@buttonstyles_horizontal_tab",
                starting_height=2,
                anchors={
        ...
        if self.the_cat.mate:
            self.tab_buttons["mates"] = UISurfaceImageButton(
                ...
                anchors={
                    "bottom": "bottom",
                    "bottom_target": self.list_frame_image,
                    "left_target": self.tab_buttons["enemies"]
                    if enemies_tab_shown
        ...
        if self.the_cat.bestie:
            anchor = self.tab_buttons["potential"]
            if mates_tab_shown:
                anchor = self.tab_buttons["mates"]
            elif enemies_tab_shown:
                anchor = self.tab_buttons["enemies"]
        ...
        if self.open_tab == "enemies" and not enemies_tab_shown:
            self.open_tab = "potential"

        self.switch_tab()

    def switch_tab(self):
        if self.open_tab == "enemies":
            self.enemies_container.show()
            self.mates_container.hide()
            self.besties_container.hide()
            self.potential_container.hide()

            if "enemies" in self.tab_buttons:
                self.tab_buttons["enemies"].disable()
            ...
            if "enemies" in self.tab_buttons:
                self.tab_buttons["enemies"].enable()
            if "besties" in self.tab_buttons:
                self.tab_buttons["besties"].enable()
            self.tab_buttons["potential"].enable()
        elif self.open_tab == "besties":
            self.enemies_container.hide()
            self.mates_container.show()
            self.potential_container.hide()
    ...
    def update_selected_cat(self):
        ...
        self.selected_cat_elements["center_heart"] = pygame_gui.elements.UIImage(
            ...
            pygame.transform.scale(
                image_cache.load_image(
                    ...
                    if self.selected_cat.ID in self.the_cat.enemy
                    else "resources/images/heart_breakup.png"
                    if self.selected_cat.ID in self.the_cat.previous_enemies
        ...
        if self.selected_cat.enemy:
            info += f"\n{len(self.selected_cat.enemy)} " + i18n.t(
                "general.enemy", count=len(self.selected_cat.enemy)
            )
        ...
        if self.selected_cat.ID in self.the_cat.enemy:
            self.toggle_enemy = UISurfaceImageButton(
                ui_scale(pygame.Rect((323, 310), (153, 30))),
                "screens.choose_enemy.unset_enemy",
                get_button_dict(ButtonStyles.SQUOVAL, (153, 30)),
                object_id="@buttonstyles_squoval",
            )
        else:
            self.toggle_enemy = UISurfaceImageButton(
                ui_scale(pygame.Rect((323, 310), (153, 30))),
                "screens.choose_enemy.set_enemy",
                get_button_dict(ButtonStyles.SQUOVAL, (153, 30)),
                object_id="@buttonstyles_squoval",
            )
